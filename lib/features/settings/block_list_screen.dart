...
class _BlockListScreenState extends State<BlockListScreen> {
  ...
  List<IBlockPosting> blockPostingList = [];
  ...
  void initState() {
    ...
    blockPostingList = BlockSet().getBlockPostingList(); 
  }
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            if (blockPostingList.isNotEmpty)
              Padding(
                padding: const EdgeInsets.all(8.0),
                child: ListView.separated(
                    shrinkWrap: true,
                    physics: const NeverScrollableScrollPhysics(),
                    itemCount: blockPostingList.length,
                    itemBuilder: (context, index) {
                      return GestureDetector(
                        onTap: () {},
                        child: Card(
                              ...
                              Expanded(
                                ...
                                children: [
                                  Padding(
                                    padding: const EdgeInsets.all(8.0),
                                    child: Text(
                                        "Title : ${blockPostingList[index].blockedTitle}"),
                                  ),
                                  Padding(
                                    padding: const EdgeInsets.all(8.0),
                                    child: Text(
                                        "Host : ${blockPostingList[index].blockedAuthorNickname}"),
                                  ),
                                ],
                              )),
                              Row(
                                children: [
                                  Padding(
                                    ...
                                    child: TextButton(
                                      ...
                                      onPressed: () async {
                                        BlockSet().removeBlockPosting(
                                            blockedPostId:
                                                blockPostingList[index]
                                                    .blockedPostId);
                                        communityPageController.refresh();
                                        setState(() {
                                          blockPostingList =
                                              BlockSet().getBlockPostingList();
                                        });
                                      },
                                    ),
                                  ),
